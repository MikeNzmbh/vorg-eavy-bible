"use strict";
var VorgSalesForecast;
(function (VorgSalesForecast) {
    VorgSalesForecast.FORECAST_VERSION = "VORG Sales Forecast v1.1";
    VorgSalesForecast.PRIOR_VERSION = "VORG cold-start priors 2026-07.1";
    ...
    VorgSalesForecast.DEFAULT_PRIORS = Object.freeze({
    ...
    VorgSalesForecast.hasEvidenceReference = hasEvidenceReference;
    function isSyntheticEvidenceReference(value) {
        const reference = String(value || "").trim();
        if (/^synthetic:\/\//i.test(reference))
            return true;
        return /(^|[\\/])synthetic(?:[-_](?:forecast|test|fixture|evidence))?(?:[\\/]|$)/i.test(reference);
    }
    VorgSalesForecast.isSyntheticEvidenceReference = isSyntheticEvidenceReference;
    function normalizeExternalPrior(input, errors) {
        ...
        if (errors.length || directConversionStrength === null || refundStrength === null)
            return null;
        return {
            id: "public-transfer-v1",
            modelVersion,
    ...
    function calculateForecast(input) {
        ...
        const priorProfile = requestedPriorProfile === "public-transfer-v1"
            ? "public-transfer-v1"
            : "internal-weak";
        ...
        const priorVersion = externalPrior
            ? `${VorgSalesForecast.PRIOR_VERSION} + ${externalPrior.modelVersion}`
            : VorgSalesForecast.PRIOR_VERSION;
        if (externalPrior) {
        ...
        const unitsPerOrderAssumption = clampNumber(unitsPerOrderInput !== null && unitsPerOrderInput !== void 0 ? unitsPerOrderInput : 1.05, 1, 4);
        ...
        const committedNonInventorySpend = Math.max(0, committedSpendInput !== null && committedSpendInput !== void 0 ? committedSpendInput : 0);
        ...
        const onlineConversionStressMultiplier = clampNumber(conversionStressInput !== null && conversionStressInput !== void 0 ? conversionStressInput : 1, 0.01, 3);
        const reservationRateInput = numberOrNull(input.reservationConversionRate);
        if (String((_a = input.reservationConversionRate) !== null && _a !== void 0 ? _a : "").trim() &&
        ...
        const reservationRate = clampNumber(reservationRateInput !== null && reservationRateInput !== void 0 ? reservationRateInput : 60, 0, 100) / 100;
        ...
        const inventoryCost = hasCompleteCosts
            ? products.reduce((sum, product) => sum + product.inventory * product.landedCogs, 0)
            : null;
        const productionSpendCap = numberOrNull(input.productionSpendCap);
        if (!products.length)
            errors.push("At least one active product with inventory and price is required.");
        if (products.length && totalInventory <= 0)
            errors.push("Recorded active inventory must be greater than zero.");
        ...
        const evidenceMode = requestedEvidenceMode === "synthetic" || syntheticReference ? "synthetic" : "live";
        ...
        const hasObservedFunnel = observed.sessions > 0 && ["sessions", "productViews", "addsToCart", "checkouts", "purchases"]
            .every(key => observed.provided[key]);
        ...
        const onlineConversionMode = !hasObservedFunnel && plannedOnlineConversionRate !== null
            ...
            : "funnel";
        if (!trafficReference)
            warnings.push("Planned traffic has no linked plan or historical receipt; traffic uncertainty is widened.");
        else if (!trafficEvidence)
            warnings.push("Planned traffic has a linked plan but no historical receipt; it remains a scenario input and receives wider uncertainty.");
        if (!hasObservedFunnel)
            warnings.push("No complete observed VORG core funnel is recorded; missing transitions use weak cold-start priors.");
        else if (!funnelEvidence)
            warnings.push("Observed funnel counts have no proof reference and remain scenario inputs.");
        if (reservations > 0 && !reservationEvidence)
            warnings.push("Reservations have no receipt and remain an assumption.");
        ...
        if (onlineConversionMode === "planning-prior") {
            ...
            if (hasSessionPurchasePair) {
                ...
                assumptions.push(`Online session-to-purchase planning prior starts at ${round(plannedOnlineConversionRate * 100, 2)}% and is updated with ${observed.purchases}/${observed.sessions} first-party orders/sessions.`);
            }
            else {
                warnings.push("Online orders use the entered session-to-purchase planning prior; it is not observed VORG conversion.");
                assumptions.push(`Online session-to-purchase planning prior: ${round(plannedOnlineConversionRate * 100, 2)}%.`);
            }
        }
        ...
        const rates = buildRateModels(observed, reservationRate, hasEvidenceReference(input.reservationEvidenceUrl), unitsPerOrderAssumption, plannedOnlineConversionRate, onlineConversionMode === "planning-prior", externalPrior);
        const rateOutputs = rateSummary(rates).map(rate => {
            if (evidenceMode === "synthetic")
                return { ...rate, evidence: "synthetic" };
            if (externalPrior && rate.evidence === "prior" && ["plannedSessionPurchase", "refundRate"].includes(rate.key)) {
                return { ...rate, evidence: "external" };
            }
            return rate;
        });
        ...
        const totalDemand = [];
        const totalSold = [];
        const totalOnlineSold = [];
        const totalPopupSold = [];
        const totalSellThrough = [];
        const totalExcess = [];
        const totalLostDemand = [];
        let sellThrough50 = 0;
        let sellThrough70 = 0;
        let sellThrough90 = 0;
        let sellThrough85 = 0;
        ...
        const productAccumulators = products.map(product => ({
            demand: [],
            sold: [],
            onlineSold: [],
            popupSold: [],
        ...
        for (let iteration = 0; iteration < simulations; iteration += 1) {
            ...
            products.forEach((product, productIndex) => {
                ...
                if (product.landedCogs !== null)
                    acc.grossProfit.push(profit);
                ...
                if (product.variants.length) {
                    product.variants.forEach((variant, variantIndex) => {
                        ...
                        if (demandForVariant > 0 && demandForVariant >= variant.inventory)
                            variantAcc.stockouts += 1;
                    });
                }
            ...
            if (simulationSellThrough >= 0.5)
                sellThrough50 += 1;
            if (simulationSellThrough >= 0.7)
                sellThrough70 += 1;
            if (simulationSellThrough >= 0.85)
                sellThrough85 += 1;
            if (simulationSellThrough >= 0.9)
                sellThrough90 += 1;
            if (simulationSellThrough >= 0.999)
                sellThrough100 += 1;
            if (simulationAnyStockout)
                anyStockout += 1;
            if (inventoryCost !== null && simulationRevenue >= inventoryCost)
                inventoryCashRecovered += 1;
            if (inventoryCost !== null && simulationRevenue >= inventoryCost + committedNonInventorySpend)
                launchCashRecovered += 1;
        }
        const productOutputs = products.map((product, productIndex) => {
            const acc = productAccumulators[productIndex];
            return {
                id: product.id,
                name: product.name,
        ...
        return {
            version: VorgSalesForecast.FORECAST_VERSION,
            priorVersion,
            priorProfile,
    ...
    VorgSalesForecast.calculateForecast = calculateForecast;
    function calculateCalibration(snapshots, mode = "live") {
        const validCandidates = (Array.isArray(snapshots) ? snapshots : []).filter(snapshot => {
            var _a, _b, _c, _d, _e, _f;
            const actualRevenue = numberOrNull((_a = snapshot.actual) === null || _a === void 0 ? void 0 : _a.revenue);
            const actualUnits = numberOrNull((_b = snapshot.actual) === null || _b === void 0 ? void 0 : _b.unitsSold);
            const snapshotIsSynthetic = ((_c = snapshot.forecast) === null || _c === void 0 ? void 0 : _c.status) === "synthetic-test" || ((_d = snapshot.forecast) === null || _d === void 0 ? void 0 : _d.evidenceMode) === "synthetic" ||
                isSyntheticEvidenceReference((_e = snapshot.actual) === null || _e === void 0 ? void 0 : _e.evidenceUrl);
            ...
            return belongsToMode && hasCalibratableForecast(snapshot.forecast) && actualRevenue !== null && actualRevenue >= 0 &&
                actualUnits !== null && actualUnits >= 0 && hasEvidenceReference((_f = snapshot.actual) === null || _f === void 0 ? void 0 : _f.evidenceUrl);
        });
        ...
        validCandidates.forEach(snapshot => {
            var _a;
            const dropKey = String(snapshot.dropId || ((_a = snapshot.forecast) === null || _a === void 0 ? void 0 : _a.dropId) || "unknown");
        ...
        valid.forEach(snapshot => {
            ...
            if (actualRevenue >= forecast.summary.revenue.p10 && actualRevenue <= forecast.summary.revenue.p90)
                revenueCovered += 1;
            if (actualUnits >= forecast.summary.soldUnits.p10 && actualUnits <= forecast.summary.soldUnits.p90)
                unitsCovered += 1;
            const actualSellThrough = numberOrNull((_c = snapshot.actual) === null || _c === void 0 ? void 0 : _c.sellThroughPct);
        ...
        if (uniqueDrops < 4)
            warnings.push("Calibration is directional until at least four independent drops have linked outcomes.");
        if (validCandidates.length > uniqueDrops)
            warnings.push("Multiple snapshots from one drop are useful audits; only the earliest timestamped linked call enters calibration metrics.");
    ...
    VorgSalesForecast.calculateCalibration = calculateCalibration;
    function calculateStressSuite(input) {
        var _a;
        ...
        const fixedSpend = Math.max(0, (_a = numberOrNull(input.committedNonInventorySpend)) !== null && _a !== void 0 ? _a : 0);
        ...
        const scenario = (key, label, description, overrides, productCostScale = 1, seedOffset = 1) => {
            const products = (input.products || []).map(product => ({
                ...product,
                landedCogs: productCostScale === 1 || numberOrNull(product.landedCogs) === null
                    ? product.landedCogs
                    : numberOrNull(product.landedCogs) * productCostScale,
        ...
        return [
            scenario("traffic-miss", "Traffic miss", "Online and pop-up traffic finish 30% below plan.", { plannedOnlineSessions: Math.round(sessions * 0.7), plannedPopupVisitors: Math.round(popupVisitors * 0.7) }, 1, 11),
            scenario("popup-cancelled", "Pop-up cancelled", "The physical event contributes zero visitors or sales.", { plannedPopupVisitors: 0 }, 1, 23),
            scenario("cost-overrun", "Cost overrun", "Landed COGS rises 20% and committed non-inventory spend rises 10%.", { committedNonInventorySpend: fixedSpend * 1.1 }, 1.2, 37),
            scenario("combined-downside", "Combined downside", "Traffic misses by 30%, purchase conversion compresses by 35%, the pop-up contributes zero, COGS rises 20%, and other committed spend rises 10%.", {
            ...
            scenario("upside", "Controlled upside", "Traffic beats plan by 20% and purchase conversion improves by 30%; inventory still caps sales.", {
    ...
    VorgSalesForecast.calculateStressSuite = calculateStressSuite;
    function hasCalibratableForecast(value) {
        var _a, _b, _c, _d, _e, _f;
        const summary = value === null || value === void 0 ? void 0 : value.summary;
        const required = [
            (_a = summary === null || summary === void 0 ? void 0 : summary.revenue) === null || _a === void 0 ? void 0 : _a.p10,
            (_b = summary === null || summary === void 0 ? void 0 : summary.revenue) === null || _b === void 0 ? void 0 : _b.p50,
            (_c = summary === null || summary === void 0 ? void 0 : summary.revenue) === null || _c === void 0 ? void 0 : _c.p90,
            (_d = summary === null || summary === void 0 ? void 0 : summary.soldUnits) === null || _d === void 0 ? void 0 : _d.p10,
            (_e = summary === null || summary === void 0 ? void 0 : summary.soldUnits) === null || _e === void 0 ? void 0 : _e.p50,
            (_f = summary === null || summary === void 0 ? void 0 : summary.soldUnits) === null || _f === void 0 ? void 0 : _f.p90,
            summary === null || summary === void 0 ? void 0 : summary.sellThrough70Probability
        ];
        return required.every(item => typeof item === "number" && Number.isFinite(item));
    }
    function normalizeObserved(input, errors) {
        const provided = {
        ...
        sequence.forEach(([bothProvided, successes, trials, message]) => {
            if (bothProvided && successes > trials)
                errors.push(message);
        });
    ...
    function normalizeProducts(input, errors, warnings) {
        return input.filter(product => product.active !== false).map((product, index) => {
            ...
            const enteredWeight = String((_a = product.weight) !== null && _a !== void 0 ? _a : "").trim();
            const parsedWeight = positiveNumber(product.weight);
            if (enteredWeight && parsedWeight === null)
                errors.push(`${name}: demand weight must be greater than zero.`);
            const weight = parsedWeight !== null && parsedWeight !== void 0 ? parsedWeight : 1;
            const variants = (product.variants || []).map((variant, variantIndex) => {
                var _a;
                const inventory = integerOrNull(variant.inventory);
                const variantWeightEntered = String((_a = variant.weight) !== null && _a !== void 0 ? _a : "").trim();
                ...
                return {
                    ...
                    inventory,
                    weight: parsedVariantWeight !== null && parsedVariantWeight !== void 0 ? parsedVariantWeight : 1
                };
            }).filter((variant) => variant !== null);
            const variantInventory = variants.reduce((sum, variant) => sum + variant.inventory, 0);
            if (variants.length && variants.some((_, variantIndex) => { var _a, _b; return !String((_b = (_a = (product.variants || [])[variantIndex]) === null || _a === void 0 ? void 0 : _a.weight) !== null && _b !== void 0 ? _b : "").trim(); })) {
            ...
            if (!inventoryProvided)
                errors.push(`${name}: inventory must be a non-negative whole number.`);
            if (price === null || price <= 0)
                errors.push(`${name}: price must be greater than zero.`);
            if (landedCogs !== null && landedCogs < 0)
                errors.push(`${name}: landed COGS cannot be negative.`);
            if (landedCogs !== null && price !== null && landedCogs >= price)
                warnings.push(`${name}: landed COGS is at or above price; forecast profit can be zero or negative.`);
            ...
            return {
                id,
                name,
                inventory: Math.max(0, inventory !== null && inventory !== void 0 ? inventory : 0),
                price: Math.max(0, price !== null && price !== void 0 ? price : 0),
    ...
    function buildRateModels(observed, reservationRate, reservationEvidence, unitsPerOrderAssumption, plannedOnlineConversionRate, useDirectPlanningRate, externalPrior) {
        var _a;
        ...
        const directStrength = (_a = externalPrior === null || externalPrior === void 0 ? void 0 : externalPrior.directConversionStrength) !== null && _a !== void 0 ? _a : INTERNAL_DIRECT_PRIOR_STRENGTH;
        const refundPriorMean = VorgSalesForecast.DEFAULT_PRIORS.refundRate.a / (VorgSalesForecast.DEFAULT_PRIORS.refundRate.a + VorgSalesForecast.DEFAULT_PRIORS.refundRate.b);
        const refundPriorA = externalPrior ? refundPriorMean * externalPrior.refundStrength : VorgSalesForecast.DEFAULT_PRIORS.refundRate.a;
        const refundPriorB = externalPrior ? (1 - refundPriorMean) * externalPrior.refundStrength : VorgSalesForecast.DEFAULT_PRIORS.refundRate.b;
        return {
            sessionToView: betaPosteriorWhenProvided(observed.productViews, observed.sessions, observed.provided.productViews && observed.provided.sessions, VorgSalesForecast.DEFAULT_PRIORS.sessionToView.a, VorgSalesForecast.DEFAULT_PRIORS.sessionToView.b),
            viewToCart: betaPosteriorWhenProvided(observed.addsToCart, observed.productViews, observed.provided.addsToCart && observed.provided.productViews, VorgSalesForecast.DEFAULT_PRIORS.viewToCart.a, VorgSalesForecast.DEFAULT_PRIORS.viewToCart.b),
            cartToCheckout: betaPosteriorWhenProvided(observed.checkouts, observed.addsToCart, observed.provided.checkouts && observed.provided.addsToCart, VorgSalesForecast.DEFAULT_PRIORS.cartToCheckout.a, VorgSalesForecast.DEFAULT_PRIORS.cartToCheckout.b),
            checkoutToPurchase: betaPosteriorWhenProvided(observed.purchases, observed.checkouts, observed.provided.purchases && observed.provided.checkouts, VorgSalesForecast.DEFAULT_PRIORS.checkoutToPurchase.a, VorgSalesForecast.DEFAULT_PRIORS.checkoutToPurchase.b),
            ...
            popupPurchaseRate: betaPosteriorWhenProvided(observed.popupPurchases, observed.popupVisitors, observed.provided.popupPurchases && observed.provided.popupVisitors, VorgSalesForecast.DEFAULT_PRIORS.popupPurchaseRate.a, VorgSalesForecast.DEFAULT_PRIORS.popupPurchaseRate.b),
            ...
            plannedSessionPurchase: plannedOnlineConversionRate === null
                ? null
                : buildPlanningConversion(plannedOnlineConversionRate, directStrength, observed),
            unitsPerOrder,
    ...
    function betaPosterior(successes, trials, priorA, priorB) {
        return {
            a: priorA + successes,
    ...
    function betaPosteriorWhenProvided(successes, trials, provided, priorA, priorB) {
        if (!provided || trials <= 0)
            return { a: priorA, b: priorB, successes: -1, trials: 0 };
        return betaPosterior(successes, trials, priorA, priorB);
    }
    function buildPlanningConversion(plannedOnlineConversionRate, directStrength, observed) {
    ...
    function completenessScore(input) {
        ...
        const observedFields = ["sessions", "productViews", "addsToCart", "checkouts", "purchases"]
            .filter(key => input.observed.provided[key]).length / 5;
        const evidenceCoverage = ((input.trafficEvidence ? 1 : 0) +
            (input.funnelEvidence ? 1 : 0) +
            (input.reservations === 0 || input.reservationEvidence ? 1 : 0)) / 3;
        return Math.round(clampNumber(financialCoverage * 40 + trafficPresent * 15 + observedFields * 25 + evidenceCoverage * 15 + sizeCoverage * 5, 0, 100));
    }
    function blockedOutput(input, horizonDays, simulations, seed, dataCompleteness, unitsPerOrder, onlineConversionMode, evidenceMode, priorProfile, priorVersion, rates, errors, warnings, assumptions) {
        var _a;
        ...
        return {
            ...
            summary: {
                ...
                committedNonInventorySpend: Math.max(0, (_a = numberOrNull(input.committedNonInventorySpend)) !== null && _a !== void 0 ? _a : 0),
    ...
    function percentiles(values, digits = 0) {
        if (!values.length)
    ...
    function sampleBinomial(n, p, rng) {
        ...
        if (trials === 0 || probabilityValue === 0)
            return 0;
    ...
    function sampleGamma(shape, rng) {
        ...
        while (true) {
            ...
            if (uniform < 1 - 0.0331 * normal ** 4)
                return d * v;
            if (Math.log(uniform) < 0.5 * normal ** 2 + d * (1 - v + Math.log(v)))
                return d * v;
        }
    }
    ...
    function probability(count, total) {
    ...
    function validatedNonNegativeInteger(value, label, errors) {
    ...
    function numberOrNull(value) {
        ...
        const normalized = String(value !== null && value !== void 0 ? value : "").trim().replace(/[$,]/g, "").replace(/^C\s*/i, "");
        if (!normalized)
            return null;
...
})(VorgSalesForecast || (VorgSalesForecast = {}));

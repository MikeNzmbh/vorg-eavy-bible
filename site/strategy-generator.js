 ...
 */
var VorgStrategyGenerator;
(function (VorgStrategyGenerator) {
    VorgStrategyGenerator.ENGINE_VERSION = "VORG Strategy Generator v1.1";
    VorgStrategyGenerator.DEFAULT_SELL_THROUGH_GOAL_PCT = 85;
    ...
    function transferFactor(cardIndustry) {
        const industry = (cardIndustry || "").toLowerCase();
        if (SAME_INDUSTRY.indexOf(industry) >= 0)
            return 1.0;
        if (ADJACENT_INDUSTRY.indexOf(industry) >= 0)
            return 0.8;
        return 0.6;
    }
    VorgStrategyGenerator.transferFactor = transferFactor;
    function isCrossIndustry(cardIndustry) {
        return transferFactor(cardIndustry) < 1.0;
    }
    function usableCard(card) {
        const checkedAt = Date.parse(card.dateChecked);
        return (hasEvidenceReference(card.sourceUrl) &&
    ...
    function sourceClassFactor(sourceClass) {
        ...
        if (/vendor-case-study/.test(normalized))
            return 0.6;
        if (/brand-primary/.test(normalized))
            return 0.55;
    ...
    function sourceAgeFactor(dateChecked, asOf) {
        ...
        if (!Number.isFinite(checkedAt) || !Number.isFinite(decisionAt))
            return 0;
        if (checkedAt > decisionAt + 24 * 60 * 60 * 1000)
            return 0;
    ...
    function qualityAdjustedCard(card, asOf) {
        if (!usableCard(card))
            return null;
    ...
    function gateAppliesToMarkets(gate, markets) {
        ...
        return markets.some((market) => {
            if (/UNITED STATES|\bUS\b|\bUSA\b/i.test(market.country))
                return region.includes("US");
            if (/CANADA|\bCA\b/i.test(market.country))
                return region.includes("CA");
            return region === "GLOBAL";
        });
    }
    function toEvidenceUse(card, weight) {
        const use = {
            cardId: card.id,
    ...
    function supportingWeight(archetype, library) {
        let w = 0;
        for (const card of library) {
            if (!usableCard(card))
                continue;
            if (archetype.mechanisms.indexOf(card.mechanism) < 0)
                continue;
            const cw = card.confidence * transferFactor(card.industry);
            if (card.workedOrFailed === "worked")
                w += cw;
            if (card.workedOrFailed === "mixed")
                w += cw * 0.5;
        }
        return w;
    }
    function generateStrategySpace(library) {
        const space = ARCHETYPES.map((a) => ({
            id: a.id,
            label: a.label,
        ...
        for (const a of ARCHETYPES) {
            for (const otherId of a.hybridCompatible) {
                ...
                if (supportingWeight(a, library) <= 0 || supportingWeight(b, library) <= 0)
                    continue;
    ...
    function stressTest(strategy, library, gates) {
        const supporting = [];
        const contradicting = [];
        ...
        const sourceCardCounts = new Map();
        ...
        for (const card of eligibleCards) {
            ...
            if (matches) {
                if (card.workedOrFailed === "worked")
                    supporting.push(toEvidenceUse(card, w));
                else if (card.workedOrFailed === "failed")
                    contradicting.push(toEvidenceUse(card, w));
                else {
        ...
        for (const id of strategy.requiredGateIds)
            relevantGateIds.add(id);
        const openGateIds = [];
        for (const g of gates) {
            if (relevantGateIds.has(g.id) && gateOpen(g))
                openGateIds.push(g.id);
        }
        openGateIds.sort();
        ...
        for (const reqId of strategy.requiredGateIds) {
            ...
            if (g)
                frozenCapabilities.push(`${g.label} not cleared (${g.id})`);
        }
    ...
    function assessStrategy(strategy, library, gates) {
        const stress = stressTest(strategy, library, gates);
        let supportW = 0;
        for (const s of stress.supporting)
            supportW += s.weight;
        let contraW = 0;
        for (const c of stress.contradicting)
            contraW += c.weight;
        // Failure evidence lowers the score with reasons but never zeroes a supported strategy.
        let raw = supportW - 0.5 * contraW;
        if (supportW > 0)
            raw = Math.max(raw, 0.2 * supportW);
        if (supportW <= 0)
            raw = Math.max(raw, 0);
        let evidenceScore = clamp(raw * 20);
        if (strategy.hybridOf)
            evidenceScore = clamp(evidenceScore * HYBRID_EVIDENCE_DISCOUNT);
        ...
        if (stress.contradicting.length) {
            reasons.push(`Failure evidence lowered (did not zero) this strategy: ${stress.contradicting.map((c) => c.reference).join("; ")}.`);
        }
        const assessment = {
            strategyId: strategy.id,
        ...
        if (strategy.hybridOf)
            assessment.hybridOf = [...strategy.hybridOf];
        return assessment;
    }
    // ---------------------------------------------------------------- stage: plan compilation
    function templateFor(mechanism) {
        for (const t of WAVE_TEMPLATES)
            if (t.mechanism === mechanism)
                return t;
        return null;
    }
    function cardsForMechanism(mechanism, library) {
        return library.filter((c) => usableCard(c) && c.mechanism === mechanism);
    }
    function freezeStateFor(gateDependencies, gates) {
    ...
    function buildActionItem(idPrefix, template, library, gates, budgetCapCad) {
        ...
        if (freeze.frozenReason)
            item.frozenReason = freeze.frozenReason;
        return item;
    }
    function compilePlan(winner, input, generatedAt) {
        var _a, _b;
        const brand = input.brand;
        const gates = input.gates || [];
        const library = input.library || [];
        const goal = (_a = brand.sellThroughGoalPct) !== null && _a !== void 0 ? _a : VorgStrategyGenerator.DEFAULT_SELL_THROUGH_GOAL_PCT;
        const pool = Math.max(0, (_b = brand.testCashPoolCad) !== null && _b !== void 0 ? _b : 0);
        ...
        const p0Items = openHard.map((g, i) => ({
            id: `p0-gate-${g.id}`,
            action: `Clear gate "${g.label}" with evidence (current state: ${g.state}).${g.notes ? " " + g.notes : ""}`,
            ownerRole: (g.id.indexOf("tax") >= 0 || g.id.indexOf("gst") >= 0 ? "finance" : "ops"),
            budgetCapCad: 0,
            measurement: {
        ...
        for (const { mechanism } of selected) {
            ...
            if (freeze.frozenReason)
                wave.frozenReason = freeze.frozenReason;
            ...
            items: [
                {
                    ...
                    evidenceCardIds: cardsForMechanism("under-supplied-drop", library)
                        ...
                        .map((c) => c.id),
                    evidenceLabel: cardsForMechanism("under-supplied-drop", library).length ||
    ...
    // ---------------------------------------------------------------- stage: remix mode
    function remixPlan(existing, winner, input, generatedAt) {
        var _a, _b, _c, _d;
        const improvements = [];
        const gates = input.gates || [];
        const library = input.library || [];
        const brand = input.brand;
        const pool = Math.max(0, (_b = (_a = brand.testCashPoolCad) !== null && _a !== void 0 ? _a : existing.testCashPoolCad) !== null && _b !== void 0 ? _b : 0);
        const goal = (_c = brand.sellThroughGoalPct) !== null && _c !== void 0 ? _c : VorgStrategyGenerator.DEFAULT_SELL_THROUGH_GOAL_PCT;
        const asOf = generatedAt.slice(0, 10);
        const waves = [];
        ...
        for (const raw of existingWaves) {
            ...
            for (const ri of rawItems) {
                ...
                let label = validCards.length ? "evidence-backed" : "working-assumption";
                ...
                const measurement = {
                    ...
                    receiptRequired: true,
                    successThreshold: (ri.measurement && ri.measurement.successThreshold) || "explicit threshold to be set by owner before launch",
                ...
                const item = {
                    ...
                    ownerRole: ri.ownerRole || "founder",
                    budgetCapCad: budget,
                    measurement,
            ...
            if (waveFrozen)
                wave.frozenReason = (_d = items.filter((i) => i.frozenReason)[0]) === null || _d === void 0 ? void 0 : _d.frozenReason;
            waves.push(wave);
        }
        ...
        if (totalBudget > pool) {
            ...
            for (const w of waves) {
                ...
                for (const item of w.items)
                    item.budgetCapCad = Math.floor(item.budgetCapCad * scale);
        ...
        const phaseIds = [];
        for (const w of waves)
            if (phaseIds.indexOf(w.phaseId) < 0)
                phaseIds.push(w.phaseId);
        phaseIds.sort();
    ...
    function calibrate(plan, winner, results, goal) {
        const auditTrail = [];
        ...
        const byWave = new Map(plan.waves.map((w) => [w.id, w]));
        let passes = 0;
        let fails = 0;
        let freed = 0;
        const scaledWaves = [];
    ...
    // ---------------------------------------------------------------- main entry
    function generateStrategy(rawInput) {
        var _a, _b, _c, _d;
        ...
        let mode = "from-scratch";
        let plan = null;
        let remixImprovements = [];
        if (winner) {
        ...
        const unpreparednessReport = gates
            .filter((g) => gateOpen(g))
            .map((g) => ({
            gateId: g.id,
            label: g.label,
        ...
        const assumptions = [
            ...
            `Test cash pool C$${Math.max(0, (_b = brand.testCashPoolCad) !== null && _b !== void 0 ? _b : 0)} is a founder-configurable working assumption, fully separate from the C$${brand.productionCeilingCad.min}-C$${brand.productionCeilingCad.max} production ceiling.`,
        ...
        if (input.macro && (((_c = input.macro.headwinds) === null || _c === void 0 ? void 0 : _c.length) || ((_d = input.macro.tailwinds) === null || _d === void 0 ? void 0 : _d.length))) {
            assumptions.push(`Macro context supplied (${(input.macro.headwinds || []).length} headwind(s), ${(input.macro.tailwinds || []).length} tailwind(s)) — treated as planning context, not demand evidence.`);
        }
        return {
            version: VorgStrategyGenerator.ENGINE_VERSION,
            generatedAt,
            mode,
            goalSellThroughPct: goal,
            winner,
    ...
    VorgStrategyGenerator.generateStrategy = generateStrategy;
    // Exported for tests
    VorgStrategyGenerator.__test = {
        clamp,
        round2,
        transferFactor,
...
})(VorgStrategyGenerator || (VorgStrategyGenerator = {}));

namespace VorgSupplierVetting {
  export const ALGORITHM_VERSION = "VORG Supplier Vetting v1.0";
  export const PRIMARY_PLATFORM = "Alibaba.com";

  export type StyleId = "VE-FJ-001" | "VE-WD-001" | "VE-MD-001" | "VE-WT-001" | "VE-SC-001";
  export type EvidenceStatus = "claimed" | "linked" | "verified" | "contradicted";
  ...
  export type EvidenceRequirement =
    ...
    | "wash-or-specialist-site"
    | "no-substitution-commitment"
    ...
    | "test-results"
    | "pp-sample";
  export type Gate = "reject" | "screening" | "rfq-ready" | "sample-candidate" | "founder-review";
  export type MaxAuthorizedAction =
    | "none"
    | "draft-message"
    | "request-rfq"
  ...
  export interface SupplierRiskSignals {
    identityMismatch?: boolean;
    beneficiaryMismatch?: boolean;
    refusesLiveVerification?: boolean;
    refusesInspection?: boolean;
    hiddenSubcontracting?: boolean;
    pressuresBulkBeforeSample?: boolean;
    requestsPersonalPayment?: boolean;
    requestsOffPlatformPayment?: boolean;
    fabricatedEvidence?: boolean;
    unapprovedSubstitution?: boolean;
    wantsOffPlatformChatOnly?: boolean;
    evasiveAnswers?: boolean;
    impossibleLeadTime?: boolean;
    customMoqUnclear?: boolean;
    catalogImagesOnly?: boolean;
  }
  ...
  export interface SupplierEvaluation {
    ...
    categoryScores: Record<EvidenceCategory, number>;
    gate: Gate;
    maxAuthorizedAction: MaxAuthorizedAction;
    hardStops: string[];
    warnings: string[];
  ...
  export interface DraftedMessage {
    ...
    body: string;
    attachments: string[];
    evidenceToLog: EvidenceRequirement[];
    doNotSendIf: string[];
  }
  ...
  export const STYLE_CHALLENGES: Readonly<Record<StyleId, { label: string; proofAsk: string; technicalAsk: string }>> = {
    "VE-FJ-001": {
  ...
  const SAMPLE_REQUIREMENTS: EvidenceRequirement[] = [
    "proto-measurement",
    "sample-construction",
    "physical-fit",
    "test-results",
  ...
  export function evaluateSupplier(record: SupplierRecord): SupplierEvaluation {
    ...
    for (const definition of REQUIREMENTS) {
      ...
      rawScore += awarded;
      if (best < 0.8) missingRequirements.push(definition.requirement);
    }
    ...
    const hasIndependentEvidence = (record.evidence || []).some(item =>
      item.status === "verified" && ["platform-report", "live-challenge", "third-party", "physical-sample"].includes(item.authority)
    );
    const hasCompleteSampleProof = SAMPLE_REQUIREMENTS.every(requirement => requirementIsStrong(record, requirement));

    if (!hasIndependentEvidence) score = Math.min(score, 44);
    if (!hasCompleteSampleProof) score = Math.min(score, 74);
    if (contradictions.length > 0 && hardStops.length === 0) score = Math.min(score, 49);
    if (hardStops.length > 0) score = 0;
  ...
  export function nextAction(record: SupplierRecord): NextAction {
  ...
  export function draftNextMessage(record: SupplierRecord, now = new Date()): DraftedMessage {
    const next = nextAction(record);
    const styleId = record.styleIds[0] || "VE-FJ-001";
    const style = STYLE_CHALLENGES[styleId];
  ...
  export function findContradictions(claims: SupplierClaim[]): Contradiction[] {
    const groups = new Map<string, SupplierClaim[]>();
    for (const claim of claims) {
      const key = normalize(claim.key);
      if (!key || !normalize(claim.value)) continue;
  ...
  export function proofCode(record: SupplierRecord, now = new Date()): string {
  ...
  function determineGate(record: SupplierRecord, score: number, hardStops: string[], contradictions: Contradiction[]): Gate {
    if (hardStops.length > 0) return "reject";
    const hasMaterialContradiction = contradictions.some(item => item.material);
    if (hasMaterialContradiction) return "reject";

    const rfqCore = ["legal-identity", "similar-front-back-inside", "construction-answer"] as EvidenceRequirement[];
    const sampleCore = [
      "legal-identity",
      "live-facility",
      "subcontractor-map",
  ...
  function actionForGate(gate: Gate): MaxAuthorizedAction {
    if (gate === "reject") return "none";
    if (gate === "screening") return "draft-message";
    if (gate === "rfq-ready") return "request-rfq";
    if (gate === "sample-candidate") return "prepare-sample-order";
    return "prepare-bulk-review";
  }

  function evidenceValue(item: EvidenceItem): number {
    if (item.status === "contradicted") return 0;
    const statusMultiplier: Record<EvidenceStatus, number> = {
      claimed: 0.15,
      linked: 0.55,
      verified: 1,
      contradicted: 0
    };
    const authorityMultiplier: Record<EvidenceAuthority, number> = {
      "supplier-claim": 0.7,
    ...
    let provenance = 1;
    if (!usable(item.capturedAt)) provenance *= 0.7;
    if ((item.status === "linked" || item.status === "verified") && !usable(item.sourceUrl) && !usable(item.details)) provenance *= 0.4;
    return clamp(statusMultiplier[item.status] * authorityMultiplier[item.authority] * provenance, 0, 1);
  }

  function requirementIsPresent(record: SupplierRecord, requirement: EvidenceRequirement): boolean {
    return (record.evidence || []).some(item => item.requirement === requirement && evidenceValue(item) >= 0.35);
  }

  function requirementIsStrong(record: SupplierRecord, requirement: EvidenceRequirement): boolean {
    return (record.evidence || []).some(item => item.requirement === requirement && evidenceValue(item) >= 0.8);
  }

  function collectHardStops(risks: SupplierRiskSignals, contradictions: Contradiction[]): string[] {
    const hardStops: string[] = [];
    if (risks.identityMismatch) hardStops.push("Legal identity does not reconcile across the profile, licence or quotation.");
    if (risks.beneficiaryMismatch) hardStops.push("The requested payment beneficiary does not match the approved commercial entity or platform order.");
    if (risks.refusesLiveVerification) hardStops.push("Supplier refused a reasonable live dated capability check.");
    if (risks.refusesInspection) hardStops.push("Supplier refused independent inspection.");
    if (risks.hiddenSubcontracting) hardStops.push("Supplier concealed or refused to identify a production subcontractor.");
    if (risks.pressuresBulkBeforeSample) hardStops.push("Supplier pressured VORG-EAVY to commit to bulk before the sample ladder.");
    if (risks.requestsPersonalPayment) hardStops.push("Supplier requested payment to a personal or undocumented account.");
    if (risks.requestsOffPlatformPayment) hardStops.push("Supplier requested payment outside the protected Alibaba order path.");
    if (risks.fabricatedEvidence) hardStops.push("Supplier evidence appears fabricated or materially misrepresented.");
    if (risks.unapprovedSubstitution) hardStops.push("Supplier made or proposed an unapproved substitution as if it were equivalent.");
  ...
  function collectWarnings(record: SupplierRecord, contradictions: Contradiction[]): string[] {
    const risks = record.risks || {};
    const warnings: string[] = [];
    if (risks.wantsOffPlatformChatOnly) warnings.push("Supplier wants key discussion off-platform; preserve the controlling record in Alibaba Message Center.");
    if (risks.evasiveAnswers) warnings.push("Supplier did not answer numbered questions directly.");
    if (risks.impossibleLeadTime) warnings.push("Claimed lead time needs a capacity and milestone reality check.");
    if (risks.customMoqUnclear) warnings.push("Listed MOQ may be stock or blank MOQ rather than the custom style MOQ.");
    if (risks.catalogImagesOnly) warnings.push("Catalogue images do not prove that this entity or facility made the garments.");
  ...
  function action(kind: NextActionKind, label: string, rationale: string, requiredEvidence: EvidenceRequirement[]): NextAction {
    return { kind, label, rationale, requiredEvidence, requiresActionTimeApproval: true };
  }

  function emptyCategoryScores(): Record<EvidenceCategory, number> {
    return {
      identity: 0,
      capability: 0,
      "similar-work": 0,
  ...
  function roundCategoryScores(scores: Record<EvidenceCategory, number>): Record<EvidenceCategory, number> {
    return {

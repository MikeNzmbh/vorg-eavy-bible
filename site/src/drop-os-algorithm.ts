namespace VorgDropAlgorithm {
  ...
  export function statusMultiplier(status: unknown): number {
    ...
    if (status === "in progress") return 0.65;
    if (status === "blocked") return 0.18;
    return 0.25;
  }
  ...
  export function calculateScores(input: DropScoreInput): ScoreOutput {
    ...
    const positiveInputs = STRESS_LABELS.filter(item => item.type === "positive");
    const inputScore = positiveInputs.reduce(
      (sum, item) => sum + getStressValue(stress, operationsEffective, productProofScore, item.key) * item.weight,
      0
    );
    ...
    const stageScore = average(input.stages.map(stage =>
      coerceNumber(stage.score) * statusMultiplier(stage.status) * gateMultiplier(stage.gate)
    ));
  ...
  export function scoreCitySignals(signals: SignalScoreInput[], cities?: string[]): CitySignalScore[] {
    ...
    signals.forEach(signal => {
      ...
      byCity.get(city)?.push(coerceNumber(signal.strength));
    });
  ...
  function citySignalScore(values: number[]): number {
  ...
  function getStressValue(
    ...
    if (key === "operations") return operationsEffective;
    if (key === "product") return productProofScore;
    return stress[key];
  }
